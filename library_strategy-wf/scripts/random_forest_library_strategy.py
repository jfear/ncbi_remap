...
def main():
    ...
    # Split into training and test data keeping class proportions similar (stratify)
    ...
    train_sizes, train_scores, test_scores = learning_curve(
        clf, X_train, Y_train, cv=5, train_sizes=np.linspace(0.001, 1, 20), n_jobs=-1
    )
    ...
    for i in range(n):
        X_train, X_test, Y_train, Y_test = train_test_split(X, Y_enc, stratify=Y_enc, test_size=0.8)
        clf.fit(X_train, Y_train)
