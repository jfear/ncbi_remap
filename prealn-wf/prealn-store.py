...
def munge_big_data():
    ...
    dask_run_large_data(Path(WORKDIR, '../output/prealn-wf/samtools_idx_stats/{srx}.parquet').as_posix(),
                        patterns['samtools_idxstats'],
                        munge_as_dataframe,
                        parse_samtools_idxstats)

    logger.info(f'Building counts files')
    dask_run_large_data(Path(WORKDIR, '../output/prealn-wf/gene_counts/{srx}.parquet').as_posix(),
                        patterns['feature_counts']['counts'],
                        munge_as_dataframe,
                        parse_featureCounts_counts)

    logger.info(f'Building junction counts files')
    dask_run_large_data(Path(WORKDIR, '../output/prealn-wf/junction_counts/{srx}.parquet').as_posix(),
