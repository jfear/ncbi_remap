...
def parse_fastq_screen(sample, file):
    ...
    with open(file, 'r') as fh:
        header = ['sample', 'reference', 'category', 'type', 'value']
        parsed = []
        ...
        for l in fh:
            ...
            if fqs:
                org = fqs.group(1)
                parsed.append((sample, org, 'reads_processed', 'count', int(fqs.group(2))))
                parsed.append((sample, org, 'unmapped', 'count', int(fqs.group(3))))
                parsed.append((sample, org, 'unmapped', 'percent', float(fqs.group(4))))
                parsed.append((sample, org, 'one_hit_one_library', 'count', int(fqs.group(5))))
                parsed.append((sample, org, 'one_hit_one_library', 'percent', float(fqs.group(6))))
                parsed.append((sample, org, 'multiple_hits_one_library', 'count', int(fqs.group(7))))
                parsed.append((sample, org, 'multiple_hits_one_library', 'percent', float(fqs.group(8))))
                parsed.append((sample, org, 'one_hit_multiple_libraries', 'count', int(fqs.group(9))))
                parsed.append((sample, org, 'one_hit_multiple_libraries', 'percent', float(fqs.group(10))))
                parsed.append((sample, org, 'multiple_hits_multiple_libraries', 'count', int(fqs.group(11))))
                parsed.append((sample, org, 'multiple_hits_multiple_libraries', 'percent', float(fqs.group(12))))

        if len(parsed) == 0:
            return None
        else:
            df = pd.DataFrame(parsed, columns=header)
            return df.set_index(['sample', 'reference', 'category', 'type'])
